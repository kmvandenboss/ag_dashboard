#!/usr/bin/env python3
"""
CFTC Options Data for Soybeans - Futures and Options Combined
Downloads CFTC Disaggregated COT reports (F&O Combined) to extract options positioning data
Calculates put/call ratios and trader positioning in soybean options
...
START_YEAR = 2006
OUT = "data/cftc_options_soybean.csv"
...
def filter_soybean_options(df):
    """
    Filter for soybeans and extract combined futures+options positioning data
    ...
    if d.empty:
        print("Warning: No soybean data found in this file")
        return pd.DataFrame()
...
def main():
    ...
    for url in year_urls():
        try:
            ...
            dfy = parse_zip(buf)
            if not dfy.empty:
                filtered = filter_soybean_options(dfy)
    ...
    if not frames:
        raise SystemExit("No CFTC options data fetched. Check if soybeans is in Supplemental reports.")
