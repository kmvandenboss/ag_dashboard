...
def fetch_crop_progress():
    ...
    df = fetch_nass_data(params, "Crop progress (PLANTED, EMERGED, SILKING, etc.)")
    ...
    # Example: "CORN - PROGRESS, MEASURED IN PCT PLANTED"
    df['stage'] = df['short_desc'].str.extract(r'PCT (\w+)', expand=False)
...
def clean_column_names(df):
    ...
    # Standardize progress columns
    if 'planted' in df.columns:
        rename_map['planted'] = 'pct_planted'
    if 'emerged' in df.columns:
        rename_map['emerged'] = 'pct_emerged'
    if 'silking' in df.columns:
        rename_map['silking'] = 'pct_silking'
    if 'dough' in df.columns:
        rename_map['dough'] = 'pct_dough'
    if 'dented' in df.columns:
        rename_map['dented'] = 'pct_dented'
    if 'mature' in df.columns:
        rename_map['mature'] = 'pct_mature'
...
def main():
    print("="*80)
    print("FETCHING CROP CONDITIONS DATA FROM USDA NASS")
    print("="*80)
    print(f"\nData source: USDA NASS QuickStats API")
    print(f"Date range: {START_YEAR} to present")
    print(f"Coverage: National corn crop conditions and progress")
    print(f"Frequency: Weekly reports during growing season (Apr-Nov)")
    print("="*80)
